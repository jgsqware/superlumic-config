...
if [ -d "/usr/local/superlumic/config" ]; then
    ...
    if [ ! -z "$repo" ]; then
        ...
        git clone -q $2 /usr/local/superlumic/config
    else
...
if [ -f "config/$playbook.yml" ]; then
    setStatusMessage "Running the ansible playbook for $playbook"
    ansible-playbook -i "localhost," config/$playbook.yml
else
    if [ "travis" = "$playbook" ]; then
        setStatusMessage "Running the ansible playbook for $playbook but use roderik.yml as fallback"
        ansible-playbook -i "localhost," config/roderik.yml
    else
        triggerError "No playbook for $playbook found"
    fi
fi
